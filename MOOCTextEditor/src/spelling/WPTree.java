...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		visited.add(word1);
		queue.add(root);
		while(!queue.isEmpty() && nw.dict.isWord(word2) && visited.size()<99999999){
			...
			for(String n:currList){
				if(!visited.contains(n)){
					curr.addChild(n);
					visited.add(n);
					WPTreeNode nTreeNode = new WPTreeNode(n, curr);
					queue.add(nTreeNode);
					if(n.equals(word2)){
						//System.out.println(visited.size());
						return nTreeNode.buildPathToRoot();
					}
				}
			}
	...
	public static void main(String[] args) {
		...
		List<String> ls = wpTree.findPath(word1, word2);
