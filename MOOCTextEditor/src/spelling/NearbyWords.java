...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index < s.length()+1; index++){
			for(int codeChar = (int)'a'; codeChar <= (int)'z'; codeChar++){
				String sb = new String();
				
				sb = s.substring(0,index)+(char)codeChar+s.substring(index,s.length());
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index < s.length(); index++){
			...
			sb = s.substring(0,index) + s.substring(index+1,s.length());
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(!queue.isEmpty() && retList.size()<numSuggestions){
			...
			for(String n:currList){
				if(!visited.contains(n)){
					visited.add(n);
					queue.add(n);
					if(this.dict.isWord(n)){
						retList.add(n);
