...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		int[][] matrixMul = new int[getNumVertices()][getNumVertices()];
		for(int i=0; i<getNumVertices(); i++){
			for(int j=0; j<getNumVertices(); j++){
				matrixMul[i][j] = 0;
				for(int m=0; m<getNumVertices(); m++){
					matrixMul[i][j] += adjMatrix[i][m] * adjMatrix[m][j];
				}
			}
		}
		for(int i=0; i<getNumVertices(); i++){
			for(int j=0; j<matrixMul[v][i];j++){
				twoHop.add(i);
			}
		}
		System.out.println(twoHop.size());
		return twoHop;
	}
